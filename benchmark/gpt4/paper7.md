<untrusted-content source="arxiv" id="2303.08774">
...
We are committed to independent auditing of our technologies, and shared some initial steps and ideas in this area in the system card accompanying this release.11In addition to the accompanying system card, OpenAI will soon publish additional thoughts on the social and economic implications of AI systems, including the need for effective regulation. We plan to make further technical details available to additional third parties who can advise us on how to weigh the competitive and safety considerations above against the scientific value of further transparency.
...
: Performance of GPT-4 and smaller models. The metric is mean log pass rate on a subset of the HumanEval dataset. A power law fit to the smaller models (excluding GPT-4) is shown as the dotted line; this fit accurately predicts GPT-4’s performance. The x-axis is training compute normalized so that GPT-4 is 1.

For an individual problem in HumanEval, performance may occasionally worsen with scale. Despite these challenges, we find an approximate power law relationship −EP​[log⁡(pass​_​rate​(C))]=α∗C−k where k and α are positive constants, and P is a subset of problems in the dataset. We hypothesize that this relationship holds for all problems in this dataset. In practice, very low pass rates are difficult or impossible to estimate, so we restrict to problems P and models M such that given some large sample budget, every problem is solved at least once by every model.
...
: Performance of GPT-4 and smaller models on the Hindsight Neglect task. Accuracy is shown on the y-axis, higher is better. ada, babbage, and curie refer to models available via the OpenAI API [47].
...
AMC 1022For AMC 10 and AMC 12 2022 exams, the human percentiles are not yet published, so the reported numbers are extrapolated and likely have wide uncertainty. See Appendix A.5.
...
AMC 1222For AMC 10 and AMC 12 2022 exams, the human percentiles are not yet published, so the reported numbers are extrapolated and likely have wide uncertainty. See Appendix A.5.
...
: GPT performance on academic and professional exams. In each case, we simulate the conditions and scoring of the real exam. We report GPT-4’s final score graded according to exam-specific rubrics, as well as the percentile of test-takers achieving GPT-4’s score.

We tested GPT-4 on a diverse set of benchmarks, including simulating exams that were originally designed for humans.33We used the post-trained RLHF model for these exams. We did no specific training for these exams. A minority of the problems in the exams were seen by the model during training; for each exam we run a variant with these questions removed and report the lower score of the two. We believe the results to be representative. For further details on contamination (methodology and per-exam statistics), see Appendix C.
...
We also evaluated the pre-trained base GPT-4 model on traditional benchmarks designed for evaluating language models. For each benchmark we report, we ran contamination checks for test data appearing in the training set (see Appendix D for full details on per-benchmark contamination).44During our contamination check we discovered that portions of BIG-bench [48] were inadvertently mixed into the training set, and we excluded it from our reported results. We used few-shot prompting [1] for all benchmarks when evaluating GPT-4.55For GSM-8K, we include part of the training set in GPT-4’s pre-training mix (see Appendix E for details). We use chain-of-thought prompting [11] when evaluating.
...
GPT-4 substantially improves over previous models in the ability to follow user intent [63]. On a dataset of 5,214 prompts submitted to ChatGPT [64] and the OpenAI API [47], the responses generated by GPT-4 were preferred over the responses generated by GPT-3.5 on 70.2% of prompts.66We collected user prompts sent to us through ChatGPT and the OpenAI API, sampled one response from each model, and sent these prompts and responses to human labelers. The labelers were instructed to judge whether the response is what the user would have wanted given the prompt. The labelers were not told which response was generated by which model and the order in which the responses were presented was randomised. We filter out prompts containing any kind of disallowed or sensitive content, including personally identifiable information (PII), sexual content, hate-speech, and similar content. We also filter short (e.g. ”Hello, ChatGPT!”) and overly-common prompts.

We are open-sourcing OpenAI Evals77https://github.com/openai/evals, our framework for creating and running benchmarks for evaluating models like GPT-4 while inspecting performance sample by sample. Evals is compatible with existing benchmarks, and can be used to track performance of models in deployment. We plan to increase the diversity of these benchmarks over time to represent a wider set of failure modes and a harder set of tasks.
...
: Performance of GPT-4 on TruthfulQA. Accuracy is shown on the y-axis, higher is better. We compare GPT-4 under zero-shot prompting, few-shot prompting, and after RLHF fine-tuning. GPT-4 significantly outperforms both GPT-3.5 and Anthropic-LM from Bai et al. [67].

GPT-4 generally lacks knowledge of events that have occurred after the vast majority of its pre-training data cuts off in September 202199The pre-training and post-training data contain a small amount of more recent data, and does not learn from its experience. It can sometimes make simple reasoning errors which do not seem to comport with competence across so many domains, or be overly gullible in accepting obviously false statements from a user. It can fail at hard problems the same way humans do, such as introducing security vulnerabilities into code it produces.
...
: Expert Red Teaming: Example prompt and completions from various models.
...
: Example prompt and completions for improved refusals on disallowed categories.
...
: Example prompt and completions for reduced refusals on allowed categories. Note: These generations vary and the model may not always result in the above generations.
...
: Rate of incorrect behavior on sensitive and disallowed prompts. Lower values are better. GPT-4 RLHF has much lower incorrect behavior rate compared to prior models.
...
Core contributors1010All author lists sorted alphabetically.
...
Compute cluster scaling1010All author lists sorted alphabetically.
...
Data1010All author lists sorted alphabetically.
...
Distributed training infrastructure1010All author lists sorted alphabetically.
...
Hardware correctness1010All author lists sorted alphabetically.

Greg Brockman, Shantanu Jain, Kyle Kosic, Michael Petrov, Nikolas Tezak, Amin Tootoonchian, Chelsea Voss, Qiming Yuan

Optimization & architecture1010All author lists sorted alphabetically.
...
Training run babysitting1010All author lists sorted alphabetically.
...
Long context research1010All author lists sorted alphabetically.

Mo Bavarian, Gabriel Goh, Heewoo Jun, Łukasz Kaiser, Chak Ming Li, Ben Wang, Clemens Winter

Long context kernels1010All author lists sorted alphabetically.
...
Architecture research1010All author lists sorted alphabetically.
...
Alignment data1010All author lists sorted alphabetically.
...
Deployment & post-training1010All author lists sorted alphabetically.
...
Dataset contributions1010All author lists sorted alphabetically.
...
Data infrastructure1010All author lists sorted alphabetically.
...
ChatML format1010All author lists sorted alphabetically.
...
Model safety1010All author lists sorted alphabetically.
...
Refusals1010All author lists sorted alphabetically.
...
Foundational RLHF and InstructGPT work1010All author lists sorted alphabetically.
...
Flagship training runs1010All author lists sorted alphabetically.
...
Code capability1010All author lists sorted alphabetically.
...
OpenAI Evals library1010All author lists sorted alphabetically.
...
Model-graded evaluation infrastructure1010All author lists sorted alphabetically.
...
Acceleration forecasting1010All author lists sorted alphabetically.

Alan Hickey, Daniel Kokotajlo, Cullen O’Keefe, Sarah Shoker

ChatGPT evaluations1010All author lists sorted alphabetically.
...
Capability evaluations1010All author lists sorted alphabetically.
...
Coding evaluations1010All author lists sorted alphabetically.
...
Real-world use case evaluations1010All author lists sorted alphabetically.

Andrew Kondrich, Joe Palermo, Boris Power, Ted Sanders

Contamination investigations1010All author lists sorted alphabetically.
...
Instruction following and API evals1010All author lists sorted alphabetically.

Diogo Almeida, Carroll Wainwright, Marvin Zhang

Novel capability discovery1010All author lists sorted alphabetically.
...
Vision evaluations1010All author lists sorted alphabetically.

Shixiang Shane Gu, Shengli Hu, Jamie Kiros, Hyeonwoo Noh, Raul Puri, Rowan Zellers

Economic impact evaluation1010All author lists sorted alphabetically.

Tyna Eloundou, Sam Manning, Aalok Mehta, Pamela Mishkin

Non-proliferation, international humanitarian law & national security red teaming1010All author lists sorted alphabetically.

Sarah Shoker

Overreliance analysis1010All author lists sorted alphabetically.

Miles Brundage, Michael Lampe, Pamela Mishkin

Privacy and PII evaluations1010All author lists sorted alphabetically.

Michael Lampe, Vinnie Monaco, Ashley Pantuliano

Safety and policy evaluations1010All author lists sorted alphabetically.
...
OpenAI adversarial testers1010All author lists sorted alphabetically.
...
System card & broader impacts analysis1010All author lists sorted alphabetically.
...
Inference research1010All author lists sorted alphabetically.

Paul Baltescu, Scott Gray, Yuchen He, Arvind Neelakantan, Michael Wu

GPT-4 API & ChatML deployment1010All author lists sorted alphabetically.

Greg Brockman, Brooke Chan, Chester Cho, Atty Eleti, Rachel Lim, Andrew Peng, Michelle Pokrass, Sherwin Wu

GPT-4 web experience1010All author lists sorted alphabetically.
...
Inference infrastructure1010All author lists sorted alphabetically.
...
Reliability engineering1010All author lists sorted alphabetically.
...
Trust & safety engineering1010All author lists sorted alphabetically.

Jeff Belgum, Madelaine Boyd, Vik Goel

Trust & safety monitoring and response1010All author lists sorted alphabetically.
...
Trust & safety policy1010All author lists sorted alphabetically.

Rosie Campbell, Kim Malfacini, Andrea Vallone, Dave Willner

Deployment compute1010All author lists sorted alphabetically.

Peter Hoeschele, Evan Morikawa

Product management1010All author lists sorted alphabetically.

Jeff Harris, Joanne Jang, Angela Jiang

Additional contributions

Sam Altman, Katie Mayer, Bob McGrew, Mira Murati, Ilya Sutskever, Peter Welinder1010All author lists sorted alphabetically.

Blog post & paper content1010All author lists sorted alphabetically.
...
Communications1010All author lists sorted alphabetically.
...
Compute allocation support1010All author lists sorted alphabetically.

Theresa Lopez, Elizabeth Tseng

Contracting, revenue, pricing, & finance support1010All author lists sorted alphabetically.

Brooke Chan, Denny Jin, Billie Jonn, Patricia Lue, Kyla Sheppard, Lauren Workman

Launch partners & product operations1010All author lists sorted alphabetically.
...
Legal1010All author lists sorted alphabetically.

Jake Berdine, Che Chang, Sheila Dunning, Ashley Pantuliano

Security & privacy engineering1010All author lists sorted alphabetically.
...
System administration & on-call support1010All author lists sorted alphabetically.

Morgan Grafstein, Francis Real, Mario Saltarelli

Authorship & credit attribution1010All author lists sorted alphabetically.
...
: Comparison between GPT-4 base and GPT-4 post-RLHF on exam benchmarks. Averaged across all exams, the base model achieves an average score of 73.7% while the RLHF model achieves an average score of 74.0%, which suggests that post-training does not substantially alter base model capability.
...
Non-contaminated GPT-4 (no vision)

GPT-4

Non-contaminated GPT-4
...
A highly knowledgeable and intelligent artificial intelligence
...
: MMLU Example prompt, presented in two different languages. Note we do not translate the choice (A-D) or ‘Answer’ tokens for prompt format consistency.
...
\dnaAkAf En\30Fw\? kA aAh\?\rs?\re

A) \dnkArZ \9pLvFQyA vAtAvrZAcF rcnA krZAyA\0 r\?\8Z\2cA r\2g En\30FwA asto

B) \dnkArZ aAkAfA\8tn \9pLvFQyA mhAsAgrA\2cA r\2g \3FEwEtEb\2Ebt hoto

C) \dnkArZ vAtAvrZ \3FEwA\7mHyAn\? lhAn tr\2glA\2bF Ev\7Krt\?

D) \dnkArZ \9pLvFc\? vAtAvrZ itr sv\0 r\2gA\2nA \3FEwADA\306wyAn\? fo\8qn G\?t\?
...
: An example MMLU question translated into Marathi, Latvian, and Welsh.
...
: Example prompt demonstrating GPT-4’s visual input capability. The prompt consists of a question which requires reading information from a chart and performing computation.
...
: Example prompt demonstrating GPT-4’s visual input capability. The prompt requires image understanding.
...
: Example prompt demonstrating GPT-4’s visual input capability.
