<untrusted-content source="arxiv" id="2401.04088">
# Mixtral of Experts
...
We release both Mixtral 8x7B and Mixtral 8x7B – Instruct under the Apache 2.0 license11https://mistral.ai/news/mixtral-of-experts/, free for academic and commercial usage, ensuring broad accessibility and potential for diverse applications.
...
where (TopK​(ℓ))i:=ℓi if ℓi is among the top-K coordinates of logits ℓ∈ℝn and (TopK​(ℓ))i:=−∞ otherwise.
...
Detailed results for Mixtral, Mistral 7B and Llama 2 7B/13B/70B and Llama 1 34B22Since Llama 2 34B was not open-sourced, we report results for Llama 1 34B. are reported in Table 2.
...
Comparison of Mixtral with Llama 2 70B and GPT-3.5. Mixtral outperforms or matches Llama 2 70B and GPT-3.5 performance on most metrics.
...
Long range performance of Mixtral. (Left) Mixtral has 100% retrieval accuracy of the Passkey task regardless of the location of the passkey and length of the input sequence. (Right) The perplexity of Mixtral on the proof-pile dataset decreases monotonically as the context length increases.
...
Bias Benchmarks. Compared Llama 2 70B, Mixtral presents less bias (higher accuracy on BBQ, lower std on BOLD) and displays more positive sentiment (higher avg on BOLD).
...
Independent human evaluation conducted by LMSys is reported in Figure 633https://huggingface.co/spaces/lmsys/chatbot-arena-leaderboard and shows that Mixtral – Instruct outperforms GPT-3.5-Turbo, Gemini Pro, Claude-2.1, and Llama 2 70B chat.
...
Text samples where each token is colored with the first expert choice.
...
We evaluate the proportion of times the same expert is assigned to a token i and its following token i+1. We report whether the first chosen expert is the same, or whether the same expert is observed as first or second choice in consecutive tokens.
For reference, the expected proportion of repetitions in the case of random assignments is 18=12.5% for “First choice” and 1−68​57≈46% for “First and second choice”.
...
The gray dashed vertical line marks 18, i.e. the proportion expected with uniform sampling.
