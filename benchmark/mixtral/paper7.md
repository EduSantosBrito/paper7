...
Figure 1: Mixture of Experts Layer. Each input vector is assigned to 2 of the 8 experts by a router. The layer’s output is the weighted sum of the outputs of the two selected experts.
...
The output of the MoE module for a given input x𝑥 is determined by the weighted sum of the outputs of the expert networks, where the weights are given by the gating network’s output. i.e. given n𝑛 expert networks {E0,Ei,…,En−1}subscript𝐸0subscript𝐸𝑖…subscript𝐸𝑛1, the output of the expert layer is given by:
∑i=0n−1G​(x)i⋅Ei​(x).superscriptsubscript𝑖0𝑛1⋅𝐺subscript𝑥𝑖subscript𝐸𝑖𝑥
Here, G​(x)i𝐺subscript𝑥𝑖 denotes the n𝑛-dimensional output of the gating network for the i𝑖-th expert, and Ei​(x)subscript𝐸𝑖𝑥 is the output of the i𝑖-th expert network. If the gating vector is sparse, we can avoid computing the outputs of experts whose gates are zero. There are multiple alternative ways of implementing G​(x)𝐺𝑥 [6, 15, 35], but a simple and performant one is implemented by taking the softmax over the Top-K logits of a linear layer [28]. We use
G​(x):=Softmax​(TopK​(x⋅Wg)),assign𝐺𝑥SoftmaxTopK⋅𝑥subscript𝑊𝑔
where (TopK​(ℓ))i:=ℓiassignsubscriptTopKℓ𝑖subscriptℓ𝑖 if ℓisubscriptℓ𝑖 is among the top-K coordinates of logits ℓ∈ℝnℓsuperscriptℝ𝑛 and (TopK​(ℓ))i:=−∞assignsubscriptTopKℓ𝑖 otherwise.
...
If one increases n𝑛 while keeping K𝐾 fixed, one can increase the model’s parameter count while keeping its computational cost effectively constant.
This motivates a distinction between the model’s total parameter count (commonly referenced as the sparse parameter count), which grows with n𝑛, and the number of parameters used for processing an individual token (called the active parameter count), which grows with K𝐾 up to n𝑛.
...
In a Transformer model, the MoE layer is applied independently per token and replaces the feed-forward (FFN) sub-block of the transformer block. For Mixtral we use the same SwiGLU architecture as the expert function Ei​(x)subscript𝐸𝑖𝑥 and set K=2𝐾2. This means each token is routed to two SwiGLU sub-blocks with different sets of weights. Taking this all together, the output y𝑦 for an input token x𝑥 is computed as:
y=∑i=0n−1Softmax​(Top2​(x⋅Wg))i⋅SwiGLUi​(x).𝑦superscriptsubscript𝑖0𝑛1⋅SoftmaxsubscriptTop2⋅𝑥subscript𝑊𝑔𝑖subscriptSwiGLU𝑖𝑥
...
74.4%
Table 2: 
Comparison of Mixtral with Llama. Mixtral outperforms or matches Llama 2 70B performance on almost all popular benchmarks while using 5x fewer active parameters during inference.
• 
...
70.9%
Table 4: 
Comparison of Mixtral with Llama on Multilingual Benchmarks. On ARC Challenge, Hellaswag, and MMLU, Mixtral outperforms Llama 2 70B on 4 languages: French, German, Spanish, and Italian.
...
51.5%
56.0%
BOLD sentiment score (avg ±plus-or-minus std)
gender
0.293 ±plus-or-minus 0.073
0.323 ±plus-or-minus0.045
profession
0.218 ±plus-or-minus 0.073
0.243 ±plus-or-minus 0.087
religious_ideology
0.188 ±plus-or-minus 0.133
0.144 ±plus-or-minus 0.089
political_ideology
0.149 ±plus-or-minus 0.140
0.186 ±plus-or-minus 0.146
race
0.232 ±plus-or-minus 0.049
0.232 ±plus-or-minus 0.052
...
The gray dashed vertical line marks 1/818, i.e. the proportion expected with uniform sampling.
Here, we consider experts that are either selected as a first or second choice by the router.
A breakdown of the proportion of assignments done in each case cane be seen in Figure 9 in the Appendix.
...
Figure 8: Text samples where each token is colored with the first expert choice.
...
We evaluate the proportion of times the same expert is assigned to a token i𝑖 and its following token i+1𝑖1. We report whether the first chosen expert is the same, or whether the same expert is observed as first or second choice in consecutive tokens.
For reference, the expected proportion of repetitions in the case of random assignments is 18=12.5%18percent12.5 for “First choice” and 1−68​57≈46%16857percent46 for “First and second choice”.
...
35:7103–7114, 2022.
Figure 9: 
Proportion of tokens assigned to each expert on different subsets from The Pile dataset, separated by whether the expert was selected as first or second choice, or either. The “Either choice” case is equivalent to Figure 7.
The gray dashed vertical line marks 1818, i.e. the proportion expected with uniform sampling.
Figure 10: 
Repeated consecutive assignments per MoE layer. Repeated assignments occur a lot more often than they would with uniform assignments (materialized by the dashed lines). Patterns are similar across datasets with less repetitions for DM Mathematics.
