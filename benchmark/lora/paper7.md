...
1 Introduction
Figure 1: Our reparametrization. We only train A𝐴 and B𝐵.
...
We can freeze the shared model and efficiently switch tasks by replacing the matrices A𝐴 and B𝐵 in Figure 1, reducing the storage requirement and task-switching overhead significantly.
• 
...
We call the input and output dimension size of a Transformer layer dm​o​d​e​lsubscript𝑑𝑚𝑜𝑑𝑒𝑙.
We use Wqsubscript𝑊𝑞, Wksubscript𝑊𝑘, Wvsubscript𝑊𝑣, and Wosubscript𝑊𝑜 to refer to the query/key/value/output projection matrices in the self-attention module.
W𝑊 or W0subscript𝑊0 refers to a pre-trained weight matrix and Δ​WΔ𝑊 its accumulated gradient update during adaptation.
We use r𝑟 to denote the rank of a LoRA module.
We follow the conventions set out by (Vaswani et al., 2017; Brown et al., 2020) and use Adam (Loshchilov & Hutter, 2019; Kingma & Ba, 2017) for model optimization and use a Transformer MLP feedforward dimension df​f​n=4×dm​o​d​e​lsubscript𝑑𝑓𝑓𝑛4subscript𝑑𝑚𝑜𝑑𝑒𝑙.
2 Problem Statement
...
Suppose we are given a pre-trained autoregressive language model PΦ​(y|x)subscript𝑃Φconditional𝑦𝑥 parametrized by ΦΦ.
For instance, PΦ​(y|x)subscript𝑃Φconditional𝑦𝑥 can be a generic multi-task learner such as GPT (Radford et al., b; Brown et al., 2020) based on the Transformer architecture (Vaswani et al., 2017).
...
Each downstream task is represented by a training dataset of context-target pairs: 𝒵={(xi,yi)}i=1,..,N, where both xisubscript𝑥𝑖 and yisubscript𝑦𝑖 are sequences of tokens.
For example, in NL2SQL, xisubscript𝑥𝑖 is a natural language query and yisubscript𝑦𝑖 its corresponding SQL command; for summarization, xisubscript𝑥𝑖 is the content of an article and yisubscript𝑦𝑖 its summary.
...
maxΦ​∑(x,y)∈𝒵∑t=1|y|log​(PΦ​(yt|x,y<t))subscriptΦsubscript𝑥𝑦𝒵superscriptsubscript𝑡1𝑦logsubscript𝑃Φconditionalsubscript𝑦𝑡𝑥subscript𝑦absent𝑡
(1)
One of the main drawbacks for full fine-tuning is that for each downstream task, we learn a different set of parameters Δ​ΦΔΦ whose dimension |Δ​Φ|ΔΦ equals |Φ0|subscriptΦ0.
Thus, if the pre-trained model is large (such as GPT-3 with |Φ0|≈175​ BillionsubscriptΦ0175 Billion), storing and deploying many independent instances of fine-tuned models can be challenging, if at all feasible.
In this paper, we adopt a more parameter-efficient approach, where the task-specific parameter increment Δ​Φ=Δ​Φ​(Θ)ΔΦΔΦΘ is further encoded by a much smaller-sized set of parameters ΘΘ with |Θ|≪|Φ0|much-less-thanΘsubscriptΦ0.
The task of finding Δ​ΦΔΦ thus becomes optimizing over ΘΘ:
maxΘ​∑(x,y)∈𝒵∑t=1|y|log⁡(pΦ0+Δ​Φ​(Θ)​(yt|x,y<t))subscriptΘsubscript𝑥𝑦𝒵superscriptsubscript𝑡1𝑦subscript𝑝subscriptΦ0ΔΦΘconditionalsubscript𝑦𝑡𝑥subscript𝑦absent𝑡
(2)
In the subsequent sections, we propose to use a low-rank representation to encode Δ​ΦΔΦ that is both compute- and memory-efficient.
When the pre-trained model is GPT-3 175B, the number of trainable parameters |Θ|Θ can be as small as 0.01%percent0.01 of |Φ0|subscriptΦ0.
...
11M
Fine-Tune/LoRA
1449.4±plus-or-minus0.8
338.0±plus-or-minus0.6
19.8±plus-or-minus2.7
AdapterLsuperscriptAdapterL
1482.0±plus-or-minus1.0 (+2.2%)
354.8±plus-or-minus0.5 (+5.0%)
23.9±plus-or-minus2.1 (+20.7%)
AdapterHsuperscriptAdapterH
1492.2±plus-or-minus1.0 (+3.0%)
366.3±plus-or-minus0.5 (+8.4%)
25.8±plus-or-minus2.2 (+30.3%)
Table 1: Infernece latency of a single forward pass in GPT-2 medium measured in milliseconds, averaged over 100 trials. We use an NVIDIA Quadro RTX8000. “|Θ|Θ” denotes the number of trainable parameters in adapter layers. AdapterLsuperscriptAdapterL and AdapterHsuperscriptAdapterH are two variants of adapter tuning, which we describe in Section 5.1. The inference latency introduced by adapter layers can be significant in an online, short-sequence-length scenario. See the full study in Appendix B.
...
For a pre-trained weight matrix W0∈ℝd×ksubscript𝑊0superscriptℝ𝑑𝑘, we constrain its update by representing the latter with a low-rank decomposition W0+Δ​W=W0+B​Asubscript𝑊0Δ𝑊subscript𝑊0𝐵𝐴, where B∈ℝd×r,A∈ℝr×kformulae-sequence𝐵superscriptℝ𝑑𝑟𝐴superscriptℝ𝑟𝑘, and the rank r≪min⁡(d,k)much-less-than𝑟𝑑𝑘.
During training, W0subscript𝑊0 is frozen and does not receive gradient updates, while A𝐴 and B𝐵 contain trainable parameters.
Note both W0subscript𝑊0 and Δ​W=B​AΔ𝑊𝐵𝐴 are multiplied with the same input, and their respective output vectors are summed coordinate-wise.
For h=W0​xℎsubscript𝑊0𝑥, our modified forward pass yields:
h=W0​x+Δ​W​x=W0​x+B​A​xℎsubscript𝑊0𝑥Δ𝑊𝑥subscript𝑊0𝑥𝐵𝐴𝑥
(3)
We illustrate our reparametrization in Figure 1.
We use a random Gaussian initialization for A𝐴 and zero for B𝐵, so Δ​W=B​AΔ𝑊𝐵𝐴 is zero at the beginning of training.
We then scale Δ​W​xΔ𝑊𝑥 by αr𝛼𝑟, where α𝛼 is a constant in r𝑟.
When optimizing with Adam, tuning α𝛼 is roughly the same as tuning the learning rate if we scale the initialization appropriately.
As a result, we simply set α𝛼 to the first r𝑟 we try and do not tune it.
This scaling helps to reduce the need to retune hyperparameters when we vary r𝑟 (Yang & Hu, 2021).
...
This means that when applying LoRA to all weight matrices and training all biases222They represent a negligible number of parameters compared to weights., we roughly recover the expressiveness of full fine-tuning by setting the LoRA rank r𝑟 to the rank of the pre-trained weight matrices.
...
No Additional Inference Latency.   When deployed in production, we can explicitly compute and store W=W0+B​A𝑊subscript𝑊0𝐵𝐴 and perform inference as usual.
Note that both W0subscript𝑊0 and B​A𝐵𝐴 are in ℝd×ksuperscriptℝ𝑑𝑘.
When we need to switch to another downstream task, we can recover W0subscript𝑊0 by subtracting B​A𝐵𝐴 and then adding a different B′​A′superscript𝐵′superscript𝐴′, a quick operation with very little memory overhead.
...
In the Transformer architecture, there are four weight matrices in the self-attention module (Wq,Wk,Wv,Wosubscript𝑊𝑞subscript𝑊𝑘subscript𝑊𝑣subscript𝑊𝑜) and two in the MLP module.
We treat Wqsubscript𝑊𝑞 (or Wksubscript𝑊𝑘, Wvsubscript𝑊𝑣) as a single matrix of dimension dm​o​d​e​l×dm​o​d​e​lsubscript𝑑𝑚𝑜𝑑𝑒𝑙subscript𝑑𝑚𝑜𝑑𝑒𝑙, even though the output dimension is usually sliced into attention heads.
...
For a large Transformer trained with Adam, we reduce that VRAM usage by up to 2/323 if r≪dm​o​d​e​lmuch-less-than𝑟subscript𝑑𝑚𝑜𝑑𝑒𝑙 as we do not need to store the optimizer states for the frozen parameters.
On GPT-3 175B, we reduce the VRAM consumption during training from 1.2TB to 350GB.
With r=4𝑟4 and only the query and value projection matrices being adapted, the checkpoint size is reduced by roughly 10,000× (from 350GB to 35MB)444We still need the 350GB model during deployment; however, storing 100 adapted models only requires 350GB + 35MB * 100 ≈ 354GB as opposed to 100 * 350GB ≈ 35TB..
...
For example, it is not straightforward to batch inputs to different tasks with different A𝐴 and B𝐵 in a single forward pass, if one chooses to absorb A𝐴 and B𝐵 into W𝑊 to eliminate additional inference latency.
...
We include one such baseline reported in prior work (Li & Liang, 2021) on GPT-2, which adapts just the last two layers (FTTop2superscriptFTTop2).
...
We use lpsubscript𝑙𝑝 (resp. lisubscript𝑙𝑖) denote the number of prefix (resp. infix) tokens.
The number of trainable parameters is |Θ|=dm​o​d​e​l×(lp+li)Θsubscript𝑑𝑚𝑜𝑑𝑒𝑙subscript𝑙𝑝subscript𝑙𝑖.
...
The resulting number of trainable parameters is |Θ|=L×dm​o​d​e​l×(lp+li)Θ𝐿subscript𝑑𝑚𝑜𝑑𝑒𝑙subscript𝑙𝑝subscript𝑙𝑖, where L𝐿 is the number of Transformer layers.
...
We call this original design AdapterHsuperscriptAdapterH.
...
We call it AdapterLsuperscriptAdapterL.
This is very similar to another deign proposed in Pfeiffer et al. (2021), which we call AdapterPsuperscriptAdapterP.
We also include another baseline call AdapterDrop (Rücklé et al., 2020) which drops some adapter layers for greater efficiency (AdapterDsuperscriptAdapterD).
...
As mentioned in Section 4.2, we only apply LoRA to Wqsubscript𝑊𝑞 and Wvsubscript𝑊𝑣 in most experiments for simplicity.
The number of trainable parameters is determined by the rank r𝑟 and the shape of the original weights: |Θ|=2×L^L​o​R​A×dm​o​d​e​l×rΘ2subscript^𝐿𝐿𝑜𝑅𝐴subscript𝑑𝑚𝑜𝑑𝑒𝑙𝑟, where L^L​o​R​Asubscript^𝐿𝐿𝑜𝑅𝐴 is the number of weight matrices we apply LoRA to.
...
91.2
86.4
RoBbasesubscriptRoBbase (BitFit)*
0.1M
84.7
93.7
92.7
...
90.8
85.2
RoBbasesubscriptRoBbase (AdptDsuperscriptAdptD)*
0.3M
87.1±plus-or-minus.0
94.2±plus-or-minus.1
88.5±plus-or-minus1.1
60.8±plus-or-minus.4
93.1±plus-or-minus.1
90.2±plus-or-minus.0
71.5±plus-or-minus2.7
89.7±plus-or-minus.3
84.4
RoBbasesubscriptRoBbase (AdptDsuperscriptAdptD)*
0.9M
87.3±plus-or-minus.1
94.7±plus-or-minus.3
88.4±plus-or-minus.1
62.6±plus-or-minus.9
93.0±plus-or-minus.2
90.6±plus-or-minus.0
75.9±plus-or-minus2.2
90.3±plus-or-minus.1
85.4
RoBbasesubscriptRoBbase (LoRA)
0.3M
87.5±plus-or-minus.3
95.1±plus-or-minus.2
89.7±plus-or-minus.7
63.4±plus-or-minus1.2
93.3±plus-or-minus.3
90.8±plus-or-minus.1
86.6±plus-or-minus.7
91.5±plus-or-minus.2
87.2
RoBlargesubscriptRoBlarge (FT)*
355.0M
90.2
96.4
90.9
...
92.4
88.9
RoBlargesubscriptRoBlarge (LoRA)
0.8M
90.6±plus-or-minus.2
96.2±plus-or-minus.5
90.9±plus-or-minus1.2
68.2±plus-or-minus1.9
94.9±plus-or-minus.3
91.6±plus-or-minus.1
87.4±plus-or-minus2.5
92.6±plus-or-minus.2
89.0
RoBlargesubscriptRoBlarge (AdptPsuperscriptAdptP)††
3.0M
90.2±plus-or-minus.3
96.1±plus-or-minus.3
90.2±plus-or-minus.7
68.3±plus-or-minus1.0
94.8±plus-or-minus.2
91.9±plus-or-minus.1
83.8±plus-or-minus2.9
92.1±plus-or-minus.7
88.4
RoBlargesubscriptRoBlarge (AdptPsuperscriptAdptP)††
0.8M
90.5±plus-or-minus.3
96.6±plus-or-minus.2
89.7±plus-or-minus1.2
67.8±plus-or-minus2.5
94.8±plus-or-minus.3
91.7±plus-or-minus.2
80.1±plus-or-minus2.9
91.9±plus-or-minus.4
87.9
RoBlargesubscriptRoBlarge (AdptHsuperscriptAdptH)††
6.0M
89.9±plus-or-minus.5
96.2±plus-or-minus.3
88.7±plus-or-minus2.9
66.5±plus-or-minus4.4
94.7±plus-or-minus.2
92.1±plus-or-minus.1
83.4±plus-or-minus1.1
91.0±plus-or-minus1.7
87.8
RoBlargesubscriptRoBlarge (AdptHsuperscriptAdptH)††
0.8M
90.3±plus-or-minus.3
96.3±plus-or-minus.5
87.7±plus-or-minus1.7
66.3±plus-or-minus2.0
94.7±plus-or-minus.2
91.5±plus-or-minus.1
72.9±plus-or-minus2.9
91.5±plus-or-minus.5
86.4
RoBlargesubscriptRoBlarge (LoRA)††
0.8M
90.6±plus-or-minus.2
96.2±plus-or-minus.5
90.2±plus-or-minus1.0
68.2±plus-or-minus1.9
94.8±plus-or-minus.3
91.6±plus-or-minus.2
85.2±plus-or-minus1.1
92.3±plus-or-minus.5
88.6
DeBXXLsubscriptDeBXXL (FT)*
1500.0M
91.8
97.2
92.0
...
Runs following this more restricted setup from Houlsby et al. (2019) are labeled with ††.
...
71.0
2.47
GPT-2 M (AdapterLsuperscriptAdapterL)*
0.37M
66.3
8.41
45.0
...
71.3
2.47
GPT-2 M (AdapterHsuperscriptAdapterH)
11.09M
67.3±plus-or-minus.6
8.50±plus-or-minus.07
46.0±plus-or-minus.2
70.7±plus-or-minus.2
2.44±plus-or-minus.01
GPT-2 M (FTTop2superscriptFTTop2)*
25.19M
68.1
8.59
46.0
...
GPT-2 M (LoRA)
0.35M
70.4±plus-or-minus.1
8.85±plus-or-minus.02
46.8±plus-or-minus.2
71.8±plus-or-minus.1
2.53±plus-or-minus.02
GPT-2 L (FT)*
774.03M
68.5
8.78
...
69.9
2.45
GPT-2 L (AdapterLsuperscriptAdapterL)
0.88M
69.1±plus-or-minus.1
8.68±plus-or-minus.03
46.3±plus-or-minus.0
71.4±plus-or-minus.2
2.49±plus-or-minus.0
GPT-2 L (AdapterLsuperscriptAdapterL)
23.00M
68.9±plus-or-minus.3
8.70±plus-or-minus.04
46.1±plus-or-minus.1
71.3±plus-or-minus.2
2.45±plus-or-minus.02
GPT-2 L (PreLayer)*
0.77M
70.3
8.85
...
0.77M
70.4±plus-or-minus.1
8.89±plus-or-minus.02
46.8±plus-or-minus.2
72.0±plus-or-minus.2
2.47±plus-or-minus.02
...
89.5
50.8/27.3/43.5
GPT-3 (AdapterHsuperscriptAdapterH)
7.1M
71.9
89.8
53.0/28.9/44.8
...
2) Is the “optimal” adaptation matrix Δ​WΔ𝑊 really rank-deficient? If so, what is a good rank to use in practice?
3) What is the connection between Δ​WΔ𝑊 and W𝑊? Does Δ​WΔ𝑊 highly correlate with W𝑊? How large is Δ​WΔ𝑊 comparing to W𝑊?
...
We set a parameter budget of 18M (roughly 35MB if stored in FP16) on GPT-3 175B, which corresponds to r=8𝑟8 if we adapt one type of attention weights or r=4𝑟4 if we adapt two types, for all 96 layers.
...
Wvsubscript𝑊𝑣
Wosubscript𝑊𝑜
Wq,Wksubscript𝑊𝑞subscript𝑊𝑘
Wq,Wvsubscript𝑊𝑞subscript𝑊𝑣
Wq,Wk,Wv,Wosubscript𝑊𝑞subscript𝑊𝑘subscript𝑊𝑣subscript𝑊𝑜
Rank r𝑟
8
8
8
...
91.3
91.7
Table 5: Validation accuracy on WikiSQL and MultiNLI after applying LoRA to different types of attention weights in GPT-3, given the same number of trainable parameters. Adapting both Wqsubscript𝑊𝑞 and Wvsubscript𝑊𝑣 gives the best performance overall. We find the standard deviation across random seeds to be consistent for a given dataset, which we report in the first column.
Note that putting all the parameters in Δ​WqΔsubscript𝑊𝑞 or Δ​WkΔsubscript𝑊𝑘 results in significantly lower performance, while adapting both Wqsubscript𝑊𝑞 and Wvsubscript𝑊𝑣 yields the best result.
This suggests that even a rank of four captures enough information in Δ​WΔ𝑊 such that it is preferable to adapt more weight matrices than adapting a single type of weights with a larger rank.
7.2 What is the Optimal Rank r𝑟 for LoRA?
We turn our attention to the effect of rank r𝑟 on model performance.
We adapt {Wq,Wv}subscript𝑊𝑞subscript𝑊𝑣, {Wq,Wk,Wv,Wc}subscript𝑊𝑞subscript𝑊𝑘subscript𝑊𝑣subscript𝑊𝑐, and just Wqsubscript𝑊𝑞 for a comparison.
Weight Type
r=1𝑟1
r=2𝑟2
r=4𝑟4
...
91.5
91.4
Table 6: Validation accuracy on WikiSQL and MultiNLI with different rank r𝑟. To our surprise, a rank as small as one suffices for adapting both Wqsubscript𝑊𝑞 and Wvsubscript𝑊𝑣 on these datasets while training Wqsubscript𝑊𝑞 alone needs a larger r𝑟. We conduct a similar experiment on GPT-2 in Section H.2.
Table 6 shows that, surprisingly, LoRA already performs competitively with a very small r𝑟 (more so for {Wq,Wv}subscript𝑊𝑞subscript𝑊𝑣 than just Wqsubscript𝑊𝑞).
This suggests the update matrix Δ​WΔ𝑊 could have a very small “intrinsic rank”.666However, we do not expect a small r𝑟 to work for every task or dataset.
Consider the following thought experiment: if the downstream task were in a different language than the one used for pre-training, retraining the entire model (similar to LoRA with r=dm​o​d​e​l𝑟subscript𝑑𝑚𝑜𝑑𝑒𝑙) could certainly outperform LoRA with a small r𝑟.
To further support this finding, we check the overlap of the subspaces learned by different choices of r𝑟 and by different random seeds. We argue that increasing r𝑟 does not cover a more meaningful subspace, which suggests that a low-rank adaptation matrix is sufficient.
Subspace similarity between different r𝑟.   Given Ar=8subscript𝐴𝑟8 and Ar=64subscript𝐴𝑟64 which are the learned adaptation matrices with rank r=8𝑟8 and 6464 using the same pre-trained model, we perform singular value decomposition and obtain the right-singular unitary matrices UAr=8subscript𝑈subscript𝐴𝑟8 and UAr=64subscript𝑈subscript𝐴𝑟64.777Note that a similar analysis can be carried out with B𝐵 and the left-singular unitary matrices – we stick with A𝐴 for our experiments.
We hope to answer: how much of the subspace spanned by the top i𝑖 singular vectors in UAr=8subscript𝑈subscript𝐴𝑟8 (for 1≤i≤81𝑖8) is contained in the subspace spanned by top j𝑗 singular vectors of UAr=64subscript𝑈subscript𝐴𝑟64 (for 1≤j≤641𝑗64)?
...
(4)
where UAr=8isuperscriptsubscript𝑈subscript𝐴𝑟8𝑖 represents the columns of UAr=8subscript𝑈subscript𝐴𝑟8 corresponding to the top-i𝑖 singular vectors.
ϕ​(⋅)italic-ϕ⋅ has a range of [0,1]01, where 11 represents a complete overlap of subspaces and 00 a complete separation.
See Figure 3 for how ϕitalic-ϕ changes as we vary i𝑖 and j𝑗.
...
Figure 3: Subspace similarity between column vectors of Ar=8subscript𝐴𝑟8 and Ar=64subscript𝐴𝑟64 for both Δ​WqΔsubscript𝑊𝑞 and Δ​WvΔsubscript𝑊𝑣. The third and the fourth figures zoom in on the lower-left triangle in the first two figures. The top directions in r=8𝑟8 are included in r=64𝑟64, and vice versa.
...
Subspace similarity between different random seeds.   We further confirm this by plotting the normalized subspace similarity between two randomly seeded runs with r=64𝑟64, shown in Figure 4.
Δ​WqΔsubscript𝑊𝑞 appears to have a higher “intrinsic rank” than Δ​WvΔsubscript𝑊𝑣, since more common singular value directions are learned by both runs for Δ​WqΔsubscript𝑊𝑞, which is in line with our empirical observation in Table 6.
...
7.3 How Does the Adaptation Matrix Δ​WΔ𝑊 Compare to W𝑊?
We further investigate the relationship between Δ​WΔ𝑊 and W𝑊.
In particular, does Δ​WΔ𝑊 highly correlate with W𝑊? (Or mathematically, is Δ​WΔ𝑊 mostly contained in the top singular directions of W𝑊?) Also, how “large” is Δ​WΔ𝑊 comparing to its corresponding directions in W𝑊?
This can shed light on the underlying mechanism for adapting pre-trained language models.
To answer these questions, we project W𝑊 onto the r𝑟-dimensional subspace of Δ​WΔ𝑊 by computing U⊤​W​V⊤superscript𝑈top𝑊superscript𝑉top, with U𝑈/V𝑉 being the left/right singular-vector matrix of Δ​WΔ𝑊. Then, we compare the Frobenius norm between ‖U⊤​W​V⊤‖Fsubscriptnormsuperscript𝑈top𝑊superscript𝑉top𝐹 and ‖W‖Fsubscriptnorm𝑊𝐹.
As a comparison, we also compute ‖U⊤​W​V⊤‖Fsubscriptnormsuperscript𝑈top𝑊superscript𝑉top𝐹 by replacing U,V𝑈𝑉 with the top r𝑟 singular vectors of W𝑊 or a random matrix.
r=4𝑟4
r=64𝑟64
Δ​WqΔsubscript𝑊𝑞
Wqsubscript𝑊𝑞
...
Wqsubscript𝑊𝑞
Random
‖U⊤​Wq​V⊤‖F=subscriptnormsuperscript𝑈topsubscript𝑊𝑞superscript𝑉top𝐹absent
0.32
21.67
0.02
1.90
37.71
0.33
‖Wq‖F=61.95subscriptnormsubscript𝑊𝑞𝐹61.95
‖Δ​Wq‖F=6.91subscriptnormΔsubscript𝑊𝑞𝐹6.91
‖Δ​Wq‖F=3.57subscriptnormΔsubscript𝑊𝑞𝐹3.57
Table 7: The Frobenius norm of U⊤​Wq​V⊤superscript𝑈topsubscript𝑊𝑞superscript𝑉top where U𝑈 and V𝑉 are the left/right top r𝑟 singular vector directions of either (1) Δ​WqΔsubscript𝑊𝑞, (2) Wqsubscript𝑊𝑞, or (3) a random matrix. The weight matrices are taken from the 48th layer of GPT-3.
We draw several conclusions from Table 7.
First, Δ​WΔ𝑊 has a stronger correlation with W𝑊 compared to a random matrix, indicating that Δ​WΔ𝑊 amplifies some features that are already in W𝑊.
Second, instead of repeating the top singular directions of W𝑊, Δ​WΔ𝑊 only amplifies directions that are not emphasized in W𝑊.
Third, the amplification factor is rather huge: 21.5≈6.91/0.3221.56.910.32 for r=4𝑟4.
See Section H.4 for why r=64𝑟64 has a smaller amplification factor.
We also provide a visualization in Section H.3 for how the correlation changes as we include more top singular directions from Wqsubscript𝑊𝑞.
...
4) Finally, the rank-deficiency of Δ​WΔ𝑊 suggests that W𝑊 could be rank-deficient as well, which can also be a source of inspiration for future works.
References
...
We vary the input batch size, sequence length, and the adapter bottleneck dimension r𝑟.
We test two adapter designs: the original one by Houlsby et al. (2019), which we call AdapterHsuperscriptAdapterH, and a recent, more efficient variant by Lin et al. (2020), which we call AdapterLsuperscriptAdapterL.
...
Figure 5: Percentage slow-down of inference latency compared to the no-adapter (r=0𝑟0) baseline. The top row shows the result for AdapterHsuperscriptAdapterH and the bottom row AdapterLsuperscriptAdapterL. Larger batch size and sequence length help to mitigate the latency, but the slow-down can be as high as over 30% in an online, short-sequence-length scenario. We tweak the colormap for better visibility.
...
With 82​K82𝐾 examples in total, DART is a significantly larger and more complex data-to-text task compared to E2E.
...
The runs with this restricted setup are marked with ††.
...
4E-04
LoRA Config.
rq=rv=8subscript𝑟𝑞subscript𝑟𝑣8
LoRA α𝛼
8
Max Seq. Len.
512
...
0.0002
Adaptation
rq=rv=4subscript𝑟𝑞subscript𝑟𝑣4
LoRA α𝛼
32
Inference
Beam Size
...
For prefix-embedding tuning, we find the optimal lpsubscript𝑙𝑝 and lisubscript𝑙𝑖 to be 256 and 8, respectively, totalling 3.2​M3.2𝑀 trainable parameters.
We use lp=8subscript𝑙𝑝8 and li=8subscript𝑙𝑖8 for prefix-layer tuning with 20.2​M20.2𝑀 trainable parameters to obtain the overall best performance.
We present two parameter budgets for LoRA: 4.7M (rq=rv=1subscript𝑟𝑞subscript𝑟𝑣1 or rv=2subscript𝑟𝑣2) and 37.7M (rq=rv=8subscript𝑟𝑞subscript𝑟𝑣8 or rq=rk=rv=ro=2subscript𝑟𝑞subscript𝑟𝑘subscript𝑟𝑣subscript𝑟𝑜2).
...
LoRA+PrefixEmbed (LoRA+PE) combines LoRA with prefix-embedding tuning, where we insert lp+lisubscript𝑙𝑝subscript𝑙𝑖 special tokens whose embeddings are treated as trainable parameters.
For more on prefix-embedding tuning, see Section 5.1.
LoRA+PrefixLayer (LoRA+PL) combines LoRA with prefix-layer tuning.
We also insert lp+lisubscript𝑙𝑝subscript𝑙𝑖 special tokens; however, instead of letting the hidden representations of these tokens evolve naturally, we replace them after every Transformer block with an input agnostic vector.
...
AdapterLsuperscriptAdapterL
0.88M
45.7±plus-or-minus.1
0.38
0.46
AdapterLsuperscriptAdapterL
23M
47.1±plus-or-minus.1
0.39
0.45
PrefLayer
0.77M
...
.33
.53
AdapterLsuperscriptAdapterL (0.37M)
45.1
54.5
50.2
.36
...
.40
.43
AdapterLsuperscriptAdapterL (11M)
48.3
60.4
54.9
.38
...
.40
LoRA (0.35M)
46.7±plus-or-minus.4
62.1±plus-or-minus.2
55.3±plus-or-minus.2
.38
.44
.41
.46
...
.33
.42
AdapterLsuperscriptAdapterL (0.88M)
49.8±plus-or-minus.0
61.1±plus-or-minus.0
56.0±plus-or-minus.0
.38
.43
.41
.44
.35
.39
AdapterLsuperscriptAdapterL (23M)
49.2±plus-or-minus.1
64.7±plus-or-minus.2
57.7±plus-or-minus.1
.39
.46
.43
.46
...
.40
LoRA (0.77M)
48.4±plus-or-minus.3
64.0±plus-or-minus.3
57.0±plus-or-minus.1
.39
.45
.42
.45
...
89.5
PrefixEmbed
lp=32,li=8formulae-sequencesubscript𝑙𝑝32subscript𝑙𝑖8
0.4 M
55.9
84.9
lp=64,li=8formulae-sequencesubscript𝑙𝑝64subscript𝑙𝑖8
0.9 M
58.7
88.1
lp=128,li=8formulae-sequencesubscript𝑙𝑝128subscript𝑙𝑖8
1.7 M
60.6
88.0
lp=256,li=8formulae-sequencesubscript𝑙𝑝256subscript𝑙𝑖8
3.2 M
63.1
88.6
lp=512,li=8formulae-sequencesubscript𝑙𝑝512subscript𝑙𝑖8
6.4 M
55.9
85.8
PrefixLayer
lp=2,li=2formulae-sequencesubscript𝑙𝑝2subscript𝑙𝑖2
5.1 M
68.5
89.2
lp=8,li=0formulae-sequencesubscript𝑙𝑝8subscript𝑙𝑖0
10.1 M
69.8
88.2
lp=8,li=8formulae-sequencesubscript𝑙𝑝8subscript𝑙𝑖8
20.2 M
70.1
89.5
lp=32,li=4formulae-sequencesubscript𝑙𝑝32subscript𝑙𝑖4
44.1 M
66.4
89.6
lp=64,li=0formulae-sequencesubscript𝑙𝑝64subscript𝑙𝑖0
76.1 M
64.9
87.9
AdapterHsuperscriptAdapterH
...
73.4
91.7
rq=rv=1subscript𝑟𝑞subscript𝑟𝑣1
4.7 M
73.4
91.3
rq=rv=2subscript𝑟𝑞subscript𝑟𝑣2
9.4 M
73.3
91.4
rq=rk=rv=ro=1subscript𝑟𝑞subscript𝑟𝑘subscript𝑟𝑣subscript𝑟𝑜1
9.4 M
74.1
91.2
rq=rv=4subscript𝑟𝑞subscript𝑟𝑣4
...
73.7
91.3
rq=rk=rv=ro=2subscript𝑟𝑞subscript𝑟𝑘subscript𝑟𝑣subscript𝑟𝑜2
18.8 M
73.7
91.7
rq=rv=8subscript𝑟𝑞subscript𝑟𝑣8
...
73.8
91.6
rq=rk=rv=ro=4subscript𝑟𝑞subscript𝑟𝑘subscript𝑟𝑣subscript𝑟𝑜4
37.7 M
74.0
91.7
rq=rv=64subscript𝑟𝑞subscript𝑟𝑣64
301.9 M
73.6
91.4
rq=rk=rv=ro=64subscript𝑟𝑞subscript𝑟𝑘subscript𝑟𝑣subscript𝑟𝑜64
603.8 M
73.9
91.4
LoRA+PE
rq=rv=8,lp=8,li=4formulae-sequencesubscript𝑟𝑞subscript𝑟𝑣8formulae-sequencesubscript𝑙𝑝8subscript𝑙𝑖4
37.8 M
75.0
91.4
rq=rv=32,lp=8,li=4formulae-sequencesubscript𝑟𝑞subscript𝑟𝑣32formulae-sequencesubscript𝑙𝑝8subscript𝑙𝑖4
151.1 M
75.9
91.1
rq=rv=64,lp=8,li=4formulae-sequencesubscript𝑟𝑞subscript𝑟𝑣64formulae-sequencesubscript𝑙𝑝8subscript𝑙𝑖4
302.1 M
76.2
91.3
LoRA+PL
...
F.3 Low-Data Regime
To evaluate the performance of different adaptation approaches in the low-data regime. we randomly sample 100, 1k and 10k training examples from the full training set of MNLI to form the low-data MNLI-n𝑛 tasks.
In Table 16, we show the performance of different adaptation approaches on MNLI-n𝑛.
...
LoRA achieves better performance than fine-tuning on both MNLI-100 and MNLI-Full, and comparable results on MNLI-1k and MNLI-10K considering the (±0.3plus-or-minus0.3) variance due to random seeds.
Method
MNLI(m)-100
MNLI(m)-1k
MNLI(m)-10k
...
89.2
91.7
Table 16: Validation accuracy of different methods on subsets of MNLI using GPT-3 175B. MNLI-n𝑛 describes a subset with n𝑛 training examples. We evaluate with the full validation set. LoRA performs exhibits favorable sample-efficiency compared to other methods, including fine-tuning.
...
LoRA
2.00E-4
PrefixEmbed lpsubscript𝑙𝑝
16
32
64
256
Adaptation-
PrefixEmbed lisubscript𝑙𝑖
8
Specific
PrefixTune
lp=li=8subscript𝑙𝑝subscript𝑙𝑖8
LoRA
rq=rv=8subscript𝑟𝑞subscript𝑟𝑣8
Table 17: The hyperparameters used for different GPT-3 adaptation methods on MNLI(m)-n𝑛.
Appendix G Measuring Similarity Between Subspaces
In this paper we use the measure ϕ​(A,B,i,j)=ψ​(UAi,UBj)=‖UAi⊤​UB‖F2min⁡{i,j}italic-ϕ𝐴𝐵𝑖𝑗𝜓superscriptsubscript𝑈𝐴𝑖superscriptsubscript𝑈𝐵𝑗superscriptsubscriptnormsuperscriptsubscript𝑈𝐴limit-from𝑖topsubscript𝑈𝐵𝐹2𝑖𝑗 to measure the subspace similarity between two column orthonormal matrices UAi∈ℝd×isuperscriptsubscript𝑈𝐴𝑖superscriptℝ𝑑𝑖 and UBj∈ℝd×jsuperscriptsubscript𝑈𝐵𝑗superscriptℝ𝑑𝑗, obtained by taking columns of the left singular matrices of A𝐴 and B𝐵.
...
To be concrete, let the singular values of UAi⊤​UBjsuperscriptsubscript𝑈𝐴limit-from𝑖topsuperscriptsubscript𝑈𝐵𝑗 to be σ1,σ2,⋯,σpsubscript𝜎1subscript𝜎2⋯subscript𝜎𝑝 where p=min⁡{i,j}𝑝𝑖𝑗. We know that the Projection Metric Ham & Lee (2008) is defined as:
d​(UAi,UBj)=p−∑i=1pσi2∈[0,p]𝑑superscriptsubscript𝑈𝐴𝑖superscriptsubscript𝑈𝐵𝑗𝑝superscriptsubscript𝑖1𝑝superscriptsubscript𝜎𝑖20𝑝
where our similarity is defined as:
ϕ​(A,B,i,j)=ψ​(UAi,UBj)=∑i=1pσi2p=1p​(1−d​(UAi,UBj)2)italic-ϕ𝐴𝐵𝑖𝑗𝜓superscriptsubscript𝑈𝐴𝑖superscriptsubscript𝑈𝐵𝑗superscriptsubscript𝑖1𝑝superscriptsubscript𝜎𝑖2𝑝1𝑝1𝑑superscriptsuperscriptsubscript𝑈𝐴𝑖superscriptsubscript𝑈𝐵𝑗2
This similarity satisfies that if UAisuperscriptsubscript𝑈𝐴𝑖 and UBjsuperscriptsubscript𝑈𝐵𝑗 share the same column span, then ϕ​(A,B,i,j)=1italic-ϕ𝐴𝐵𝑖𝑗1.
If they are completely orthogonal, then ϕ​(A,B,i,j)=0italic-ϕ𝐴𝐵𝑖𝑗0. Otherwise, ϕ​(A,B,i,j)∈(0,1)italic-ϕ𝐴𝐵𝑖𝑗01.
...
Figure 6: Normalized subspace similarity between the column vectors of Ar=8subscript𝐴𝑟8 and Ar=64subscript𝐴𝑟64 for both Δ​WqΔsubscript𝑊𝑞 and Δ​WvΔsubscript𝑊𝑣 from the 1st, 32nd, 64th, and 96th layers in a 96-layer Transformer.
Figure 7: Normalized subspace similarity between the column vectors of Ar=64subscript𝐴𝑟64 from two randomly seeded runs, for both Δ​WqΔsubscript𝑊𝑞 and Δ​WvΔsubscript𝑊𝑣 from the 1st, 32nd, 64th, and 96th layers in a 96-layer Transformer.
H.2 Effect of r𝑟 on GPT-2
We repeat our experiment on the effect of r𝑟 (Section 7.2) in GPT-2.
Using the E2E NLG Challenge dataset as an example, we report the validation loss and test metrics achieved by different choices of r𝑟 after training for 26,000 steps.
...
H.3 Correlation between W𝑊 and Δ​WΔ𝑊
...
Intuitively, when Δ​WΔ𝑊 mostly contains task-specific directions, this quantity measures how much of them are amplified by Δ​WΔ𝑊.
As shown in Section 7.3, for r=4𝑟4, this amplification factor is as large as 20. In other words, there are (generally speaking) four feature directions in each layer (out of the entire feature space from the pre-trained model W𝑊), that need to be amplified by a very large factor 20, in order to achieve our reported accuracy for the downstream specific task. And, one should expect a very different set of feature directions to be amplified for each different downstream task.
One may notice, however, for r=64𝑟64, this amplification factor is only around 2, meaning that most directions learned in Δ​WΔ𝑊 with r=64𝑟64 are not being amplified by much.
...
In contrast, those directions in the rank-4 version of Δ​WΔ𝑊 (corresponding to r=4𝑟4) are amplified by a much larger factor 20.
